...
import org.example.byMaxChunkSizeAndNumOfParallel
...
class LoadByMaxChunkSizeAndNumOfParallelTest {
    ...
    internal suspend fun loadFile(chunkSize: Int, numOfParallelRequests: Int): List<ByteArray> {
        return org.example.loadFile(
            ...
            chunkSizeProvider = ChunkSizeDefiner.byMaxChunkSizeAndNumOfParallel(chunkSize, numOfParallelRequests),
    ...
    fun `bodySize rem chunkSize != 0, chunkSize times numOfParallelRequests lt bodySize`() = runBlocking {
        ...
        val expected = testData.chunked(4) // length ceilDiv numOfParallelRequests = 25 ceilDiv 4 = 4
        assertEquals(expected, arr.map(ByteArray::decodeToString))
    }
    ...
    fun `chunkSize gt bodySize`() = runBlocking {
        ...
        val expected = testData.chunked(7)
        assertEquals(expected, arr.map(ByteArray::decodeToString))
    }
